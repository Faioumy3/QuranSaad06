import { Teacher, Student, AttendanceRecord, StudentLog, ExportData, Message, Role } from '../types';
...
export const api = {
  ...
  getStudentLogs: async (studentCode: string): Promise<StudentLog[]> => {
    ...
    const logs = snap.docs.map(d => d.data() as StudentLog);
    // ترتيب حسب الأحدث
  ...
  getMessages: async (userId: string, userRole: Role): Promise<Message[]> => {
    const q = query(collection(db, 'messages'), where('recipientId', '==', userId));
    const snap = await getDocs(q);
    const messages = snap.docs.map(d => ({...d.data() as Message, id: d.id}));
    // ترتيب من الأحدث للأقدم
    return messages.sort((a, b) => b.timestamp - a.timestamp);
  },

  getSentMessages: async (userId: string, userRole: Role): Promise<Message[]> => {
  ...
  replyToMessage: async (parentMessageId: string, reply: Message): Promise<void> => {
    const parentRef = doc(db, 'messages', parentMessageId);
    const replyWithTimestamp = { ...reply, timestamp: Date.now() };
    await setDoc(parentRef, { replies: [replyWithTimestamp] }, { merge: true });
  },
